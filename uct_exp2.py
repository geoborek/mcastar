...
if __name__ == '__main__':
    ...
    env = grid.Environment(7, 7, grid.ACTIONS, safe=False)
    ...
    ITERATIONS = 4000
    df = mcts(state, env, qfunc, iterations=ITERATIONS, discount=False, exploration=25, log=True)
    # print(a, val)
    ...
    # qfunc = Qfunction(env, vfunc_fixed, hfunc)
    # df = brue(state, env, qfunc, iterations=ITERATIONS, log=True)
    # print(df)
    ...
    plt.figure(1)
    plt.plot(range(1,ITERATIONS+1), 6.34*np.ones(ITERATIONS), label=r'$Q^*(s_0,NE)$')
    actions = env.get_applicable(state)
    ...
    # print(env.get_applicable(grid.State(2,4)))